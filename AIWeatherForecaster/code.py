...
class WeatherForecaster:
    def __init__(self, api_key="87f6aa23f6029f0ebbbee8e11483a74e"):
    ...
    def get_weather_speech(self, weather_data):
        ...
        return (f"The current weather in {weather_data['city']}, {weather_data['country']} is {weather_data['weather']} "
    ...
    def plot_forecast(self, forecast_data):
        ...
        plt.subplot(2, 1, 1)
        plt.plot(daily.index, daily['temperature']['mean'], label='Avg Temp', marker='o')
        ...
        plt.legend()
        plt.subplot(2, 1, 2)
        plt.bar(daily.index, daily['humidity']['mean'], label='Humidity', alpha=0.6)
        plt.plot(daily.index, daily['pop']['max'], label='Chance of Rain', color='red', marker='o')
    ...
    while True:
        ...
        if 'exit' in user_input or 'quit' in user_input or 'bye' in user_input:
            ...
            if forecast:
                ...
                speak("Sorry, I couldn’t retrieve the forecast for that city.")
        else:
            speak("I didn't understand that. Please say 'current', 'forecast', or 'exit'.")
